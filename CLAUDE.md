...
```
smartsplit/
  proxy.py           # HTTP server: Starlette app, 2-mode triage (RESPOND/ENRICH), CLI
...
- **2-mode triage** — RESPOND (route to best free LLM) or ENRICH (web search first, then respond)
...
6. Add to `FREE_LLM_PRIORITY` in `registry.py` if it's a free LLM
