...
class TestCompletionsEndpoint:
    ...
    def test_simple_request_returns_200(self):
        ...
        with TestClient(app) as client:
            ...
            ctx.registry.get("groq").complete = AsyncMock(return_value="Python is a programming language.")
    ...
    def test_streaming_request_returns_sse(self):
        ...
        with TestClient(app) as client:
            ctx = client.app.state.ctx
            ctx.registry.get("groq").complete = AsyncMock(return_value="Hello world")
    ...
    def test_provider_failure_returns_error_message(self):
        ...
        with TestClient(app) as client:
            ctx = client.app.state.ctx
            ctx.registry.get("groq").complete = AsyncMock(side_effect=RuntimeError("API down"))
            ...
            assert "No provider" in content or len(content) > 0

    def test_triage_counts_updated(self):
        app = _make_app()
        with TestClient(app) as client:
            ctx = client.app.state.ctx
            ctx.registry.get("groq").complete = AsyncMock(return_value="ok")
        ...
        with TestClient(app) as client:
            ...
            ctx.registry.get("groq").complete = AsyncMock(
                side_effect=[
                    ...
                    # context injection
                    "User wants React 19 info.",
                    # summarize subtask
                    "React 19 introduces server components and actions.",
                    # synthesis
                    "React 19 brings server components, actions, and improved performance.",
                ]
            )
