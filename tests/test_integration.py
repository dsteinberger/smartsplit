...
class TestFullPipeline:
    ...
    def pipeline(self, make_config, tmp_path):
        ...
        registry.get("groq").complete = AsyncMock(
            return_value=('[{"type": "general", "content": "hello", "complexity": "low"}]', TokenUsage())
        )
        registry.get("gemini").complete = AsyncMock(return_value=("Gemini response", TokenUsage()))
        registry.get("anthropic").complete = AsyncMock(return_value=("Claude response", TokenUsage()))
    ...
    async def test_complex_code_routes_to_anthropic_in_quality_mode(self, pipeline):
        ...
        registry.get("anthropic").complete = AsyncMock(
            return_value=(
                "Here is a complex algorithm implementation with detailed explanation and working code.",
                TokenUsage(),
            )
        )
    ...
    async def test_synthesis_after_multiple_subtasks(self, pipeline):
        ...
        registry.get("groq").complete = AsyncMock(
            side_effect=[
                # 1st call: LLM domain classification
                ('["web_search", "code", "writing"]', TokenUsage()),
                # 2nd call: decompose
                (
                    json.dumps(
                        [
                ...
                ("User wants React 19 features researched and implemented.", TokenUsage()),
                # 4th call: synthesize
                ("Here is the combined answer.", TokenUsage()),
            ]
        )
