...
class TestDetectWithLlm:
    async def test_enrich_with_web_search(self, mock_registry: MagicMock):
        mock_registry.call_worker_llm.return_value = json.dumps({"decision": "enrich", "enrichments": ["web_search"]})
    ...
    async def test_enrich_with_multiple_enrichments(self, mock_registry: MagicMock):
        mock_registry.call_worker_llm.return_value = json.dumps(
    ...
    async def test_transparent_when_llm_says_transparent(self, mock_registry: MagicMock):
        mock_registry.call_worker_llm.return_value = json.dumps({"decision": "transparent", "enrichments": []})
    ...
    async def test_transparent_on_invalid_json(self, mock_registry: MagicMock):
        mock_registry.call_worker_llm.return_value = "this is not json at all"
    ...
    async def test_transparent_on_non_dict_response(self, mock_registry: MagicMock):
        mock_registry.call_worker_llm.return_value = json.dumps(["enrich", "web_search"])
        decision, enrichments = await detect_with_llm("some prompt", mock_registry)
        assert decision == TriageDecision.TRANSPARENT
        assert enrichments == []

    async def test_transparent_on_call_worker_llm_exception(self, mock_registry: MagicMock):
        mock_registry.call_worker_llm.side_effect = RuntimeError("provider unavailable")
        decision, enrichments = await detect_with_llm("some prompt", mock_registry)
        assert decision == TriageDecision.TRANSPARENT
        assert enrichments == []
