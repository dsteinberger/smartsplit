...
_KEY_PATTERN = re.compile(r"(sk-ant-|gsk_|AIza|tvly_|srp_|sk-)\S+")
...
FREE_LLM_PRIORITY = ["cerebras", "groq", "gemini", "deepseek", "openrouter", "mistral"]
...
class ProviderRegistry:
    ...
    def __init__(
        ...
        for name, pconfig in provider_configs.items():
            ...
            if cls is None:
                ...
                continue
            self._providers[name] = cls(api_key=pconfig.api_key, http=http)
    ...
    async def call_free_llm(self, prompt: str, prefer: str = "groq") -> str:
        ...
        order = [prefer] + [p for p in FREE_LLM_PRIORITY if p != prefer]
    ...
    async def call_llm(self, name: str, prompt: str) -> str:
        provider = self._providers.get(name)
        if provider is None or not isinstance(provider, LLMProvider):
